...
export async function sendEmails(date: Date, time: string, email: string, name: string): Promise<boolean> {
     ...
     if (error) {
        ...
        body: {html: `<h3>Hey there!</h3>\n<p>You are receiving this email because ${name} (${email}) is trying to book a spot in your calendar on ${date.getDate()}/${date.getMonth() + 1}/${date.getFullYear()} at ${time} (Central European Time). Please let them know whether that works for you or not!</p>\n<br>\n<p>All the best,</p>\n<br>\n<p>BookMe Bot</p>\n`, to: "astraberte9@gmail.com", subject: "You Got a BookMe Request!"}
     })
     if (error1) {
